...
This skill governs ALL subagent delegation, not just cross-provider work. Do not call the Agent or Workflow tools for multi-agent work without routing assignments through the steps below first.
...
OpenAI models are reachable through Codex CLI, not the Agent model parameter. For delegated OpenAI work, spawn a Claude bridge using `model: sonnet` and `effort: low` with this contract:
